...
% Convert curvature [1/m] to radius [m]
k_all_real = 1./k_all_real;
k_all_modeled = 1./k_all_modeled;
...
title(["Tip Position Error vs Time for Simulated" "Kinematics with No Curvature Tracking"], "FontSize",20);

% Plot the k values over time
figure
hold on
plot(t,k_all_modeled(1,:),"b", "LineStyle","-");
plot(t,k_all_modeled(2,:),"r", "LineStyle","-");
plot(t,k_all_real(1,:),"b", "LineStyle","--");
plot(t,k_all_real(2,:),"r", "LineStyle","--");
xlabel("Time [sec]", "FontSize",16);
ylabel("Tube Radii [m]", "FontSize",16);
title(["CTR Bending Radii vs Time" "for Both Real and Modeled Robots"], "FontSize",20);
legend(["Modeled Robot Tube 1", "Modeled Robot Tube 2", "Real Robot Tube 1", "Real Robot Tube 2"], "FontSize",14);
