...
filename = '02_05_23-18_05_2-tubes_in-plane-bending';
% filename = '18_04_23-15_27_2-tubes_in-plane-bending';
% filename = '14_04_23-16_51_2-tubes_in-plane-bending';
...
for ii = 1:test_points-1

    tracker_tf(:,:,ii) = inv(ee_rb_tf(:,:,1))*ee_rb_tf(:,:,ii+1);
    tracker_pos(ii,:) = [tracker_tf(1,4,ii); tracker_tf(2,4,ii); tracker_tf(3,4,ii)];

    end_eff_tf(:,:,ii) = inv(ee_fk_tf(:,:,1))*ee_fk_tf(:,:,ii+1);
    end_eff_pos(ii,:) = [end_eff_tf(1,4,ii); end_eff_tf(2,4,ii); end_eff_tf(3,4,ii)];



    pos_error(ii,:) = norm(tracker_pos(ii,:)) - norm(end_eff_pos(ii,:));
    pos_c_error(ii,:) = (tracker_pos(ii,:) - end_eff_pos(ii,:))*10^3;

end

% pos_error_t = rmse(ee_fk_pos, ee_mocap_pos);
pos_error_t = rmse(end_eff_pos, tracker_pos);
