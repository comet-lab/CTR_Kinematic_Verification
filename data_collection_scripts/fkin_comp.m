...
robot = Robot(num_tubes, plotOnTube);    % robot object (to call forward kinematics of robot)
...
for i = 1:test_points   
    ...
    if(plotOnFull)
        ...
        [lls, phi, kappa] = robot.get_lls_phi_kappa;

        lls = [q_2tubes(i,1)*10^-3, lls];
        phi = [0, phi];
        kappa = [0, kappa];

        pts_per_seg = 30;
        points = [pts_per_seg 2*pts_per_seg 3*pts_per_seg 4*pts_per_seg];

        g = robotindependentmapping(kappa', deg2rad(phi)', lls', pts_per_seg);
        d = [robot.tube1.od robot.tube1.od robot.tube1.od robot.tube2.od];
        fig = draw_ctcr_cust(g,points,d,ee_fk_tf(:,:,i),ee_rb_tf(:,:,i));
    end
