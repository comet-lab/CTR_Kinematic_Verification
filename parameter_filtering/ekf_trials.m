...
q0 = [0 0 0 0];
q_target_mean = [50 50 0 0];
q_target_stdev = [25 25 180 180];

simTime = 50;
filter_hz = 10;

processNoiseXYZMean = 5e-3;
processNoiseXYZStDev = 2e-3;

processNoiseKMean = 25e-3;
processNoiseKStDev = 10e-3;

measureNoiseXYZMean = 5e-3;
measureNoiseXYZStDev = 2e-3;

jointVarNoiseRhoMean = 3;
jointVarNoiseRhoStDev = 1;

jointVarNoiseThetaMean = 5;
jointVarNoiseThetaStDev = 2;

initialCovariance = diag([100 100 100 100 100]);
...
for i=1:num_trials
    ...
    k_real = abs(randn([1 2]) .* k_real_stdev + k_real_mean);
    ...
    k_guess = abs(randn([1 2]) .* k_guess_stdev + k_guess_mean);

    q0 = q0;
    q_target = randn([1,4]) .* q_target_stdev + q_target_mean;
    ...
    processNoise = ([processNoiseXYZ processNoiseXYZOffDiagonal processNoiseXYZOffDiagonal 0 0;
        processNoiseXYZOffDiagonal processNoiseXYZ processNoiseXYZOffDiagonal 0 0;
        processNoiseXYZOffDiagonal processNoiseXYZOffDiagonal processNoiseXYZ 0 0;
        0 0 0 processNoiseK processNoiseKOffDiagonal;
        0 0 0 processNoiseKOffDiagonal processNoiseK]) .^ 2;
    ...
    jointVarNoise = ([jointVarNoiseRho jointVarNoiseRhoOffDiagonal  0 0;
        jointVarNoiseRhoOffDiagonal jointVarNoiseRho  0 0;
        0 0 jointVarNoiseTheta jointVarNoiseThetaOffDiagonal;
        0 0 jointVarNoiseThetaOffDiagonal jointVarNoiseTheta]) .^ 2;
    ...
    saveas(figures(1), "./experiments/"+i+"-tip_pos_error.png");
    saveas(figures(1), "./experiments/"+i+"-tip_pos_error.fig");
    saveas(figures(2), "./experiments/"+i+"-curvatures.png");
    saveas(figures(2), "./experiments/"+i+"-curvatures.fig");


end

writematrix(all_vars_to_save, "./experiments/all-experiments.csv");
